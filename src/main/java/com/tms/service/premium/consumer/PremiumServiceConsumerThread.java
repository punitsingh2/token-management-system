...
public class PremiumServiceConsumerThread implements Runnable {

	private final static Logger LOGGER = LoggerFactory.getLogger(PremiumServiceConsumerThread.class);

	public PremiumServiceConsumerThread() {
		// TODO Auto-generated constructor stub
	}

	public void run() {

		KafkaConsumer<String, Resource> consumer = (KafkaConsumer<String, Resource>) PremiumServiceConsumerCreator.createConsumer();
		// subscribe to our consumer topics(s)
		consumer.subscribe(Arrays.asList(SystemConfiguration.getInstance().getPropsValue("PREMIUM_SERVICE_TOPIC")));
		while (true) {
			...
			for (final ConsumerRecord<String, Resource> record : records) {
				...
				LOGGER.info(
						...
						resource.getResourceType(), resource.getToken().getTokenId(),
